...
const addPokemonToTeam = (pokeman) => {
    ...
    if (slot.length <= 0) {
        alert("Your team is full! Try removing a pokemon before adding more.")
    } else {
...
const removePokemon = ($this) => {
    // Locate pokemon info elements
    const figure = $this.find("img");
    const idTag = $this.find(".pokeInfo p")
    const titleTag = $this.find(".pokeInfo h3")

    // Locate pokemon type elements
    const card = $this.find(".typeCard");
    const border = $this.find(".typeBorder");
    
    // Get current type classes
    const cardClass = $this.find(".typeCard").attr("class").split(' ').pop();
    const borderClass = $this.find(".typeBorder").attr("class").split(' ').pop();

    // Locate pokemon stat bar elements
    ...
    // Locate pokemon stat number elements
    ...
    // Reset data and values of pokemon info card elements
    figure.attr("src", "img/unown.png");
    idTag.text(`#??`);
    titleTag.text(`??????`)

    // Remove type classes from type elements
    card.removeClass(cardClass);
    border.removeClass(borderClass);
