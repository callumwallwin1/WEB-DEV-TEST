...
function createPokemonCard(pokemon) {
    ...
    const pokemonInnnerHTML = `
        ...
        <button class="${name} ${pokemon.id} ${pokemon.stats[0].base_stat} ${pokemon.stats[1].base_stat} ${pokemon.stats[2].base_stat} ${pokemon.stats[3].base_stat} ${pokemon.stats[4].base_stat} ${pokemon.stats[5].base_stat} ${pokemon.types[0].type.name} pkmnBtn absolute right-2">
...
function addPkmnToTeam($this) {
    var pkmnClass = $this.attr("class").split(' ')[0];
    alert(pkmnClass);
};

$("#poke_container").on('click', 'button.pkmnBtn', function(e) {
    addPkmnToTeam($(this));
    e.preventDefault();
});
